...
def compute_iou(names, num_cls, target_root, gt_root, num_threads=16, arr_=None):
    ...
    if num_threads == 1:
        ...
        for name in names:
            ...
            pred = np.asarray(Image.open(os.path.join(target_root, name+'.png')), dtype=np.int32)
...
if __name__ == '__main__':
    gt = '/data/VOCdevkit/VOC2012/SegmentationClassAug'
    target = './output'
    iou = compute_iou('./dataset/voc_list/val.txt', 21, target, gt)
